...
export function publicProviderDescription(city: string, state: string, locale: PublicLocale = "en", category?: string) {
  ...
  if (locale === "pt-br") return `${category || "Serviços locais"} em ${region}. Confira serviços, avaliações e disponibilidade pela VeroTask.`;
  if (locale === "es") return `${category || "Servicios locales"} en ${region}. Consulta servicios, reseñas y disponibilidad en VeroTask.`;
  return `${category || "Local services"} in ${region}. Compare services, verified reviews and availability through VeroTask.`;
}
