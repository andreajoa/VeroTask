...
import { canCreateEvidenceUpload, MAX_EVIDENCE_IMAGE_BYTES } from "@/lib/evidence-policy";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canCreateEvidenceUpload(access.booking.status, parsed.data.kind)) {
    return NextResponse.json({ error: "evidence_upload_not_available" }, { status: 409 });
  }

  try {
    const upload = await createEvidenceUpload({ bookingId: id, kind: parsed.data.kind, contentType: parsed.data.contentType, byteSize: parsed.data.byteSize });
