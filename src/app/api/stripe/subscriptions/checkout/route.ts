...
const schema = z.object({ businessId: z.string().uuid(), plan: z.enum(["pro", "elite"]) });
...
export async function POST(request: NextRequest) {
  ...
  const priceId = parsed.data.plan === "pro"
    ? process.env.STRIPE_PRICE_PRO_USD_MONTHLY
    : process.env.STRIPE_PRICE_ELITE_USD_MONTHLY;
  const [subscription] = await db.select().from(providerSubscriptions)
    .where(and(eq(providerSubscriptions.businessId, business.id), eq(providerSubscriptions.active, true))).limit(1);
    ...
    line_items: [{
      ...
      ...(priceId ? { price: priceId } : {
        price_data: {
          ...
          product_data: { name: `VeroTask ${definition.name}` }
        }
      })
    }],
    return_url: `${baseUrl}/dashboard/providers/${business.id}/billing/return?session_id={CHECKOUT_SESSION_ID}`,
    subscription_data: { metadata: { verotask_business_id: business.id, verotask_plan: parsed.data.plan, market: "orlando_fl" } },
    metadata: {
      verotask_business_id: business.id,
      verotask_plan: parsed.data.plan,
      monthly_price_cents: String(definition.monthlyPriceCents),
      currency: "usd",
      market: "orlando_fl"
    }
