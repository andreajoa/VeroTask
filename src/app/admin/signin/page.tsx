    ...
    <main className="grid min-h-screen place-items-center bg-slate-950 px-5 py-12 text-white">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-7 shadow-2xl backdrop-blur">
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-sky-500 text-slate-950"><ShieldCheck size={24} /></span>
          <div><div className="text-xl font-black">VeroTask</div><div className="text-xs font-bold uppercase tracking-[0.18em] text-sky-300">Control Center</div></div>
        </div>
        ...
        <form action={adminSignIn} className="mt-7 space-y-4">
          ...
          <button type="submit" className="min-h-12 w-full rounded-xl bg-sky-400 px-5 font-black text-slate-950 transition hover:bg-sky-300">Enter Control Center</button>
        </form>
        <p className="mt-5 text-xs leading-5 text-slate-400">Sessions expire after 8 hours. The password is never stored in plaintext by VeroTask.</p>
      </div>
    </main>
  );
}
