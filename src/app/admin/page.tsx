...
function Metric({ label, value, detail, icon: Icon }: { label: string; value: string; detail: string; icon: typeof Activity }) {
  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
      <div className="flex items-center justify-between gap-3"><div className="text-xs font-black uppercase tracking-[0.13em] text-slate-400">{label}</div><Icon size={18} className="text-sky-300" /></div>
...
function SparkBars({ values }: { values: number[] }) {
  ...
  return <div className="flex h-24 items-end gap-1">{values.map((value, i) => <div key={i} title={String(value)} className="min-w-0 flex-1 rounded-t bg-sky-400/70" style={{ height: `${Math.max(5, Math.round((value / max) * 100))}%` }} />)}</div>;
}
...
export default async function Page() {
  ...
  const [
    ...
    db.select({
      ...
      gmv: sql<number>`coalesce(sum(${bookings.subtotalCents}) filter (where ${bookings.stripePaymentIntentId} is not null and ${bookings.status} <> 'refunded'), 0)::int`,
      fees: sql<number>`coalesce(sum(${bookings.marketplaceFeeCents}) filter (where ${bookings.status} = 'paid_out'), 0)::int`
    ...
    db.select({
      delivered: sql<number>`count(distinct ${crmEmailEvents.sendId}) filter (where ${crmEmailEvents.eventType} = 'email.delivered')::int`,
      opened: sql<number>`count(distinct ${crmEmailEvents.sendId}) filter (where ${crmEmailEvents.eventType} = 'email.opened')::int`,
      clicked: sql<number>`count(distinct ${crmEmailEvents.sendId}) filter (where ${crmEmailEvents.eventType} = 'email.clicked')::int`
    ...
    <AdminShell active="/admin">
      <div className="mx-auto max-w-[1600px]">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><div className="text-xs font-black uppercase tracking-[0.18em] text-sky-300">Live operating intelligence</div><h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">VeroTask Control Center</h1><p className="mt-2 text-sm text-slate-400">Marketplace activity, revenue, CRM, trust, marketing and auditability in one place.</p></div>
          <div className="rounded-xl border border-sky-400/20 bg-sky-400/5 px-4 py-2 text-xs font-bold text-sky-200"><span className="mr-2 inline-block h-2 w-2 rounded-full bg-sky-400" />{activeNow?.value ?? 0} active now</div>
        </div>
        ...
        <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8">
          ...
          <Metric label="Bookings 30d" value={(bookingStats?.count ?? 0).toLocaleString()} detail="Requests created in the last 30 days" icon={BriefcaseBusiness} />
          <Metric label="GMV 30d" value={money(bookingStats?.gmv ?? 0)} detail={`${money(bookingStats?.fees ?? 0)} fees on paid-out bookings`} icon={Banknote} />
        ...
        <section className="mt-5 grid gap-5 xl:grid-cols-3">
          <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 xl:col-span-2">
            <div className="flex items-center justify-between"><div><h2 className="font-black">Traffic & booking pulse</h2><p className="mt-1 text-xs text-slate-400">Last 14 days</p></div><Activity size={20} className="text-sky-300" /></div>
            <div className="mt-6 grid gap-6 sm:grid-cols-2"><div><div className="mb-3 text-xs font-black uppercase tracking-wide text-slate-400">Sessions</div><SparkBars values={visitSeries.map((r) => r.value)} /></div><div><div className="mb-3 text-xs font-black uppercase tracking-wide text-slate-400">Bookings</div><SparkBars values={bookingSeries.map((r) => r.value)} /></div></div>
          </article>
          <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
            <div className="flex items-center justify-between"><div><h2 className="font-black">Email engagement</h2><p className="mt-1 text-xs text-slate-400">Last 30 days</p></div><MailCheck size={20} className="text-sky-300" /></div>
            <div className="mt-6 text-4xl font-black">{openRate.toFixed(1)}%</div><div className="mt-1 text-xs text-slate-400">unique reported opens / deliveries (may include proxies)</div>
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-xl bg-white/5 p-3"><div className="text-lg font-black">{emailStats?.delivered ?? 0}</div><div className="text-slate-400">Delivered</div></div><div className="rounded-xl bg-white/5 p-3"><div className="text-lg font-black">{emailStats?.opened ?? 0}</div><div className="text-slate-400">Opened</div></div><div className="rounded-xl bg-white/5 p-3"><div className="text-lg font-black">{emailStats?.clicked ?? 0}</div><div className="text-slate-400">Clicked</div></div></div>
          </article>
        </section>

        <section className="mt-5 grid gap-5 2xl:grid-cols-[1.25fr_1.25fr_.8fr]">
          <article className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]"><div className="border-b border-white/10 p-5"><h2 className="font-black">Recent sessions</h2></div><div className="divide-y divide-white/5">{recentSessions.map((s) => <a key={s.id} href={`/admin/analytics/${s.id}`} className="grid grid-cols-[1fr_auto] gap-3 p-4 hover:bg-white/[0.025]"><div><div className="text-sm font-bold">{s.city || "Unknown city"}{s.region ? `, ${s.region}` : ""} {s.countryCode || ""}</div><div className="mt-1 truncate text-xs text-slate-400">{s.exitPath || s.entryPath || "/"} · {s.deviceCategory || "unknown"}</div></div><div className="text-right"><div className="text-sm font-black">{duration(s.activeSeconds)}</div><div className="mt-1 text-[10px] text-slate-400">{s.lastSeenAt.toISOString().replace("T", " ").slice(0, 19)}Z</div></div></a>)}</div></article>
          <article className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]"><div className="border-b border-white/10 p-5"><h2 className="font-black">Recent bookings</h2></div><div className="divide-y divide-white/5">{recentBookings.map(({ booking, businessName }) => <a key={booking.id} href={`/admin/bookings?booking=${booking.id}`} className="grid grid-cols-[1fr_auto] gap-3 p-4 hover:bg-white/[0.025]"><div><div className="text-sm font-bold">{businessName}</div><div className="mt-1 text-xs text-slate-400">{booking.status.replaceAll("_", " ")} · {booking.id.slice(0, 8)}</div></div><div className="text-right text-sm font-black">{money(booking.subtotalCents)}</div></a>)}</div></article>
          <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><div className="flex items-center justify-between"><h2 className="font-black">Top clicked controls</h2><MousePointerClick size={19} className="text-sky-300" /></div><div className="mt-5 space-y-3">{topClicks.length ? topClicks.map((r) => <div key={r.label || "unlabeled"} className="flex items-center justify-between gap-3 text-sm"><span className="truncate text-slate-400">{r.label || "Unlabeled control"}</span><strong>{r.value}</strong></div>) : <p className="text-sm text-slate-400">Click data appears after visitors consent to analytics.</p>}</div></article>
        </section>
      </div>
    </AdminShell>
  );
