...
export default async function Page() {
  ...
  const customerRows = await db.select({ booking: bookings, business: businesses, service: services })
    .from(bookings)
  ...
  const providerRows = owned.length
  ...
  const merged = new Map<string, { booking: typeof bookings.$inferSelect; business: typeof businesses.$inferSelect; service: typeof services.$inferSelect | null; side: "customer" | "provider" }>();
  for (const row of customerRows) merged.set(row.booking.id, { ...row, side: "customer" });
  for (const row of providerRows) merged.set(row.booking.id, { ...row, side: "provider" });
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="container-shell py-10">
        ...
        {rows.length === 0 ? <div className="card mt-8 p-7"><p className="font-black">No booking activity yet.</p><p className="mt-2 text-sm text-[var(--muted)]">Customer bookings and jobs received by your provider profiles will appear here.</p></div> : <div className="mt-8 space-y-3">{rows.map(({ booking, business, service, side }) => <Link key={booking.id} href={`/bookings/${booking.id}`} className="card block p-5 transition hover:-translate-y-0.5"><div className="flex flex-wrap items-center justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="badge bg-[var(--background)] text-[var(--muted)]">{side}</span><span className="badge bg-[var(--brand-soft)] text-[var(--brand)]">{booking.status.replaceAll("_", " ")}</span></div><h2 className="mt-3 font-black">{service?.name ?? "Local service"}</h2><p className="mt-1 text-sm text-[var(--muted)]">{business.name} · {booking.scheduledStart.toLocaleString("en-US", { timeZone: "America/New_York", dateStyle: "medium", timeStyle: "short" })}</p></div><div className="text-right"><div className="font-black">${(booking.subtotalCents / 100).toFixed(2)}</div><div className="mt-1 text-xs text-[var(--muted)]">Open booking →</div></div></div></Link>)}</div>}
      </section>
    </main>
  );
}
