...
import { BadgeCheck, BriefcaseBusiness, Building2, CalendarClock, CreditCard, Gift, LogOut, ShieldCheck, UsersRound } from "lucide-react";
      ...
      <section className="container-shell py-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            {owned.length > 0 && <Link href="/dashboard/opportunities" className="btn-primary">Opportunities</Link>}
            <Link href="/dashboard/bookings" className="btn-secondary">Bookings & jobs</Link>
            <Link href="/rewards" className="btn-secondary">Vero Rewards</Link>
            <Link href="/request-service" className="btn-secondary">Get quotes</Link>
          </div>
        </div>
                ...
                {owned.map((business) => (
                  <div className="rounded-2xl border border-[var(--line)] p-5" key={business.id}>
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      <Link className="inline-flex items-center gap-2 text-sm font-black text-[var(--brand)]" href="/dashboard/opportunities"><BriefcaseBusiness size={16} /> Opportunities</Link>
        ...
        <section className="mt-5 grid gap-5 md:grid-cols-3">
          ...
          <Link href="/rewards" className="card block p-6 transition hover:-translate-y-0.5"><Gift size={20} className="text-[var(--brand)]" /><h2 className="mt-3 font-black">Vero Rewards</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">See credits earned from completed protected bookings and your saved professionals.</p></Link>
        </section>
      </section>
    </main>
  );
