...
export default async function BookingPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ lang?: string; requested?: string }> }) {
  ...
  return <main className="min-h-screen bg-[var(--background)]">
    <header className="border-b border-[var(--line)] bg-white"><div className="container-shell flex min-h-16 flex-wrap items-center justify-between gap-3 py-2"><Link href="/" className="flex items-center gap-2 text-xl font-black"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--brand)] text-white"><BadgeCheck size={20} /></span>VeroTask</Link><div className="flex flex-wrap items-center gap-3 text-sm"><Link href="/protection" className="inline-flex items-center gap-1.5 font-black text-[var(--brand)]"><ShieldCheck size={16} /> Payment Protection</Link><span className="text-[var(--muted)]">|</span><Link href={`/bookings/${id}?lang=en`} className={locale === "en" ? "font-black" : "text-[var(--muted)]"}>EN</Link><Link href={`/bookings/${id}?lang=pt-br`} className={locale === "pt-br" ? "font-black" : "text-[var(--muted)]"}>PT-BR</Link><Link href={`/bookings/${id}?lang=es`} className={locale === "es" ? "font-black" : "text-[var(--muted)]"}>ES</Link></div></div></header>
    <section className="container-shell space-y-6 py-8 sm:py-10">
      ...
      {access.isProvider && ["accepted", "payment_authorized"].includes(access.booking.status) && <div className="card p-6"><div className="font-black">Request accepted</div><p className="mt-2 text-sm leading-6 text-[var(--muted)]">The customer has been invited to pay the VeroTask booking fee. Do not start the service until this booking changes to confirmed. The service price is paid directly to you by the customer.</p></div>}
