    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-white/10 bg-slate-950/95 p-5 backdrop-blur xl:block">
        <Link href="/dashboard" className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-sky-400 text-slate-950"><BadgeCheck size={22} /></span>
          <div><div className="font-black">VeroTask</div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-sky-300">Control Center</div></div>
        </Link>
        <nav className="mt-8 space-y-1">
          {links.map(([href, label, Icon]) => {
            const selected = (active === href || (href === "/dashboard" && active === "/admin")) || (href !== "/dashboard" && active.startsWith(href));
            return <Link key={href} href={href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition ${selected ? "bg-sky-400 text-slate-950" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><Icon size={18} />{label}</Link>;
          })}
        </nav>
      ...
      <div className="xl:pl-64">
        <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/90 px-4 py-3 backdrop-blur xl:hidden">
          <div className="flex items-center justify-between gap-3">
            <Link href="/dashboard" className="flex items-center gap-2 font-black"><BadgeCheck size={20} className="text-sky-400" />VeroTask Control</Link>
          ...
          <nav className="mt-3 flex gap-2 overflow-x-auto pb-1">
            {links.map(([href, label]) => <Link key={href} href={href} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-bold ${(active === href || (href === "/dashboard" && active === "/admin")) || (href !== "/dashboard" && active.startsWith(href)) ? "bg-sky-400 text-slate-950" : "bg-white/5 text-slate-300"}`}>{label}</Link>)}
          </nav>
        </header>
